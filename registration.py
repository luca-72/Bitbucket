...
def call_api(url, data=None, method='GET'):
    ...
    try:
        ...
        request.add_header("Authorization", "Basic %s" % "c2F0cmVnaXN0cmF0aW9uOlByMHRlY3Qhc2F0NiE=")
...
if __name__ == '__main__':
    ...
    parser.add_option("-l", "--login", dest="login", default="satregistration", help="Login user for API Calls", metavar="LOGIN")
    parser.add_option("-p", "--password", dest="password", default="password", help="Password for specified user. Will prompt if omitted", metavar="PASSWORD")
    ...
    parser.add_option("-L", "--location", dest="location", default="Franklin", help="Title of the Location in Foreman that the host is to be associated with", metavar="LOCATION")
    ...
    parser.add_option("-o", "--organization", dest="org", default="MEDHOST", help="Name of the Organization in Foreman that the host is to be associated with", metavar="ORG")
    ...
    parser.add_option("-f", "--force", dest="force", action="store_true", help="Force registration (will erase old katello and puppet certs)", default=True)
    parser.add_option("--add-domain", dest="add_domain", action="store_true", help="Automatically add the clients domain to Foreman", default=True)
    ...
    parser.add_option("--puppet-ca-port", dest="puppet_ca_port", action="store", default="8141", help="Configure Puppet agent to use this port to connect to the CA")
    ...
    parser.add_option("--rex", dest="remote_exec", action="store_true", help="Install Foreman's SSH key for remote execution.", default=True)
